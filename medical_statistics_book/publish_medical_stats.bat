...
set SRC=export\book\medical_statistics_book.md
set OUTDIR=export\book
set DOCX=%OUTDIR%\medical_statistics_book.docx
set HTML=%OUTDIR%\medical_statistics_book.html
set PDF=%OUTDIR%\medical_statistics_book.pdf
...
"%PANDOC_EXE%" "%SRC%" -s -o "%DOCX%" --toc --toc-depth=2 --metadata title="Essential Medical Statistics: A Practical Guide for Healthcare Professionals"
if errorlevel 1 ( echo DOCX build failed. ) else ( echo DOCX written to %DOCX% )

echo [2/4] Building HTML...
"%PANDOC_EXE%" "%SRC%" -s -o "%HTML%" --toc --toc-depth=2 --metadata title="Essential Medical Statistics: A Practical Guide for Healthcare Professionals"
if errorlevel 1 ( echo HTML build failed. ) else ( echo HTML written to %HTML% )

echo [3/4] Trying XeLaTeX via Pandoc...
where xelatex >nul 2>&1
if %errorlevel%==0 (
  "%PANDOC_EXE%" "%SRC%" -s -o "%PDF%" --toc --toc-depth=2 --pdf-engine=xelatex -V mainfont="Times New Roman" -V geometry:margin=1in 1>nul 2>"%OUTDIR%\xelatex_errors.log"
)
...
REM Try browser-based conversion
echo Using browser for PDF conversion...
